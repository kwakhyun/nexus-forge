...
라인의 생산량, 품질, 가동 상태와 공정 흐름을 함께 보여줍니다. 이상 카드에서 `COATER-02` 진단 화면으로 바로 이동할 수 있습니다.

![공정 개요 화면](./docs/design/implementation-overview-final.png)

### 2. 다중 센서 시계열 진단

웹 장력, 오븐 온도, 라인 속도, 비전 결함률을 하나의 시간축에 맞춰 표시합니다. 이상 구간, 이벤트 마커, 선택 시점 값을 함께 제공해 신호 사이의 선후 관계를 읽을 수 있습니다.
...
오른쪽 레일은 `댄서 롤 편위 → 웹 장력 상승 → 엣지 웨이브 결함`의 원인 사슬과 근거 신호를 보여줍니다. 작업자는 안전 조건을 확인한 후 검증 작업을 발행하며, 관리자는 담당자에게 요청을 전달합니다.
