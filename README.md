...
한 번 종결한 데모를 다시 체험하려면 설정에서 기록을 내보낸 후 확인 문구를 입력해 초기화할 수 있습니다. 초기화는 해당 사이트의 브라우저 기록을 지우며, 결과를 확인하지 않은 작업 지시 발행 요청이 있으면 먼저 요청 결과를 확인해야 합니다.
...
2026년 9월 1일, 실제 앱의 현재 샘플러를 빌드해 10만 개 합성 시점의 차트를 측정했습니다. 아래 개선율은 같은 실행 안의 차트 조건 비교이며 전체 앱의 개선율이 아닙니다.
...
별도 60분 실행은 실시간 최신 수신→차트 반영 7,200개 표본에서 p95 303.8ms, 상태 반영→차트 반영 p95 79.2ms였습니다. 720개 체크포인트에서 보존 시점은 7,179–18,106개, 표시 시점은 1,574–1,776개로 두 상한을 지켰습니다. 차트 조작 37회가 반영됐고 이상 시점이 30분 범위를 벗어난 뒤의 포커스 7회와 최종 작업 발행은 의도대로 보류됐습니다.
...
| 검증 | 결과와 범위 |
| --- | --- |
| 현재 소스 로컬 | 웹 108개, 서버 17개 통과. Chromium E2E 58개, lint, TypeScript 검사와 전체 빌드 통과 |
...
| 업무 흐름과 레이아웃 | 현재 소스의 로컬 Chromium E2E 58개 통과. 실패/생략/재시도 0개. 최초 CI의 재시도 문제와 수정 근거도 보관 |
